...
class AccountService
{
    ...
    public function getMostRecommendedProviders()
    {
        ...
        $collcetion = collect($providers)->sortBy(function ($provider, $key) {
    ...
    public function getProvidersByLocation($location){
        $providers = User::whereHas('reviews', function ($query) {
            $query->orderBy('star', 'asc');
        })
    ...
    public function getSearchSuggestions($query){
       return User::select("name", "id")->where('name', 'LIKE', '%'. $query['q']. '%')->where('status', UserStatus::Active)->where('role', UserRole::Provider)->get();
    }
}
